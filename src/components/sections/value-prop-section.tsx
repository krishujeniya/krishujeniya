...
export const ValuePropSection: FC = () => {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {coreServices.map((service, index) => (
          <Card key={index} className={cn(
            "bg-card/80 border border-border shadow-lg flex flex-col items-center text-center p-6 group transition-all duration-300 hover:shadow-accent/10", // Added group
            ...
            <CardContent className="p-0 flex-grow">
              ...
              <div className="mt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                 <span className="text-xs text-accent font-medium">Discover Solutions &rarr;</span> {/* Changed text */}
              </div>
            </CardContent>
          </Card>
        ))}
